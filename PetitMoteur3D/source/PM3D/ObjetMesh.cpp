...
void CObjetMesh::Draw()
{
	...
	pDispositif->SetViewPortDimension(512.f, 512.f);
	...
	for (int i = 0; i < NombreSubset; ++i) {
		...
		if (indexDrawAmount) {
			...
			if (Material[SubsetMaterialIndex[i]].pTextureD3D != nullptr) {
				...
				variableTexture->SetResource(Material[SubsetMaterialIndex[i]].pTextureD3D); sp.bTex = 1;
			}
			else {
				sp.bTex = 0;
			}
...
void CObjetMesh::EcrireFichierBinaire(const IChargeur& chargeur, const std::string& nomFichier)
{
	...
	fichier.write((char*)&NombreSubsetp, sizeof(NombreSubsetp));
	...
	SubsetMI.reserve(NombreSubsetp);
	for (int32_t i = 0; i < NombreSubsetp; ++i)
	{
		int32_t index;
	...
	fichier.write((char*)SubsetMI.data(), (NombreSubsetp) * sizeof(int32_t));
}
...
void CObjetMesh::InitMatricesShadowMap()
{
	...
	const float planRapproche = 2.0f; 	// Pas besoin d'�tre trop pr�s
