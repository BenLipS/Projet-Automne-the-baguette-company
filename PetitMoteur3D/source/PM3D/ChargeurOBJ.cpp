...
namespace PM3D
{
	...
	{
		// Les indices de OBJ commencent � 1
		...
		float maxY{};
		float maxZ{};
		std::for_each(Position.begin(), Position.end(), [&sumX, &sumY, &sumZ, &minX, &minY, &minZ, &maxX, &maxY, &maxZ](XMFLOAT3& n) { 
			n.x -= sumX; 
			n.y -= sumY;
			n.z -= sumZ;
			n.x *= 100; 
	...
	bool CChargeurOBJ::LireFichier(const std::string& nomFichier)
	{
		...
		// **** Partie 1 - Lire le fichier en m�moire tampon
		if (!fichier.is_open())
		{
			return false;
		}
	...
	void CChargeurOBJ::TraiterLigneOBJ(const std::string& Instr)
	{
		...
		case 'v':	//  v?
			...
			if (leCar == ' ')	  // v - position de sommet
			{
				...
				// Si le mod�le est main droite, on inverse le Z
				Position.emplace_back(x, y, bMainGauche ? z : z * -1.0f);
			}

			if (leCar == 't')	//vt - coordonn�es d'application de texture
			{
				float tu, tv;
				...
				// Si le mod�le est main droite, on inverse le tv
		...
		case 'g':	// g - definir un groupe
			...
			// ... Mais �a pourrait �tre int�ressant pour des animations sp�cifiques.
			SubsetIndex.push_back(static_cast<int>(tabIndex.size()));  // Point de d�part des indices pour ce subset
			NombreSubset++;
		...
		case 'f':	// f - pour face (en r�alit� un triangle - presque toujours!!!)
			leCar = static_cast<char>(iss.get());		// Sauter le blanc
			leCar = static_cast<char>(iss.get());      // Premier caract�re
			...
			while (leCar != '\0')
			{
				...
				leCar = static_cast<char>(iss.get());			// Caract�re suivant
			}
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				// Les indices peuvent �tre relatifs � la fin
	...
	void CChargeurOBJ::TraiterLigneMAT(const std::string& Instr)
	{
		...
		case 'd':   // d - Tranparence , peut aussi �tre Tr
		{
		...
		case 'n':	// newmtl - Nouveau mat�riau
		{
			std::string str;
			iss >> str;
		...
		case 'N':     // Nd - Puissance de sp�cularit�
		{
			leCar = static_cast<char>(iss.get());
		...
		case 'T':   // Tr - Tranparence , peut aussi �tre d
			leCar = static_cast<char>(iss.get());
			if (leCar == 'r')
			{
				leCar = static_cast<char>(iss.get());	// Oter le blanc
