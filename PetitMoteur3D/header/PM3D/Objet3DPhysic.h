...
namespace PM3D
{
	...
	protected:
		...
		Objet3DPhysic(Scene* _scene, PxRigidActor* _body, CDispositifD3D11* _pDispositif, const std::vector<IChargeur*> chargeurs) : CObjetMesh(getChargeurLOD(chargeurs,_body),chargeurs, _pDispositif), scene_(_scene), body_(_body) {
			scene_->scene_->addActor(*body_);
		};
	public:
		~Objet3DPhysic();
		...
		IChargeur* getChargeurLODMoteur(std::vector<IChargeur*> chargeurs, PxRigidActor* body);
